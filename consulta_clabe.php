...
try {
    ...
    // Cobro pendiente más antiguo de ese alumno (una CLABE puede cubrir
    // varios cobros a lo largo del ciclo escolar, uno a la vez).
    $stmtCob = $pdo->prepare(
        "SELECT id, total FROM cobros WHERE cliente_id = ? AND estado = 'pendiente' ORDER BY id ASC LIMIT 1"
    );
    ...
    log_clabe("OK: clabe:{$clabe} cobro_id:{$cobro['id']} monto:{$monto_centavos}");
    // transaccion = id del cobro: nos permite en pago_clabe.php confirmar
    // exactamente ESTE cobro sin ambigüedad, sin depender de reparsear nada.
    responder_consulta_clabe(0, 'Operación exitosa', $monto_centavos, $clabe, intval($cobro['id']));
