...
function EstadoBadge({estado}) {
  ...
  const icons = { pagado:'✓', pendiente:'⏳', cancelado:'✕', vencido:'⚠' };
  return <span className={`badge ${map[estado]||'badge-gray'}`}>{icons[estado]||''} {estado}</span>;
}

function MetodoBadge({metodo}) {
  const map = { TC:'badge-blue', SPEI:'badge-purple', CoDi:'badge-green', Efectivo:'badge-gray' };
  const icons = { TC:'💳', SPEI:'🏦', CoDi:'📱', Efectivo:'💵' };
  return <span className={`badge ${map[metodo]||'badge-gray'}`}>{icons[metodo]||''} {metodo}</span>;
}
