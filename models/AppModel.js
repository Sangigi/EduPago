    ...
    return principales.map(esc => {
      ...
      if (!hayDetalleCompleto && resumen_escuelas) {
        const r = idsGrupo.reduce((acc, id) => {
          const s = resumen_escuelas[id] || { total_alumnos: 0, saldo_total: 0, cobrado_90d: 0, num_cobros_90d: 0 };
          acc.total_alumnos += s.total_alumnos;
          acc.saldo_total += s.saldo_total;
          acc.cobrado_90d += s.cobrado_90d || 0;
          acc.num_cobros_90d += s.num_cobros_90d || 0;
          return acc;
        }, { total_alumnos: 0, saldo_total: 0, cobrado_90d: 0, num_cobros_90d: 0 });
        return {
          escuela_id:     esc.id,
          nombre:         esc.nombre,
          clave:          esc.clave,
